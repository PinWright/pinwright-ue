...
- **Meshes, skeletons and clips have a text form too.** `.pwmodel`, `.pwskel` and `.pwanim` recipes compile into Static Meshes, skeletal rigs and animation clips, so generated content is source-controlled text with a build step, not a binary the agent cannot revisit. Fourteen mesh recipes and a rigged arm ship under [Examples/](Examples/).
...
| Level generation | Landscape terrain (grid spawn, sculpting, noise seeding), foliage, sky, fog and time of day, plus spatial reasoning: raycasts, surface placement and annotated captures so the agent places things by measurement, not guesswork (`landscape`, `environment`, `foliage`, `spatial`) | Experimental |
