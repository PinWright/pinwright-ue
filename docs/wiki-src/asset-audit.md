...
- A texture whose platform data is still compiling is deferred during folder sweeps. Direct `texture.describe` / `asset.dump` calls return `ASSET_COMPILING`; they do not serialize Unreal's temporary `32x32` / `PF_B8G8R8A8` stand-in. Folder sweeps requeue it a few dozen entries from the front of the queue and retry it there, and report `ASSET_COMPILE_TIMEOUT` only if no asset anywhere finishes compiling for 120 seconds; the skip preserves a prior dump when one exists, and says so when none does.
