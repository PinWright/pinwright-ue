...
Move to a live element's center and inject a mouse-button click. Params: `handle` (required, from a `drive.observe` element), `button` (`left` default, `right`, `middle`), `os_input` (default false, see below), plus the common settle/observe/journal params (`observe` defaults to `none`, `full_diff` defaults to false). Re-resolves the handle against the live UI (a hidden/disabled re-resolve is `TARGET_CHANGED`, not a silent no-op), clicks, then settles.

Before the button goes down, the pointer is moved to the element's center and the click waits (up to ~0.3 s) until Slate routes that point to the element's **own** top-level window. If another window still owns the point, typically a `Message Log` or plugin window the editor opened over a PIE viewport, the click is refused with `TARGET_OCCLUDED`: the payload's `occluding_window` names that window, and nothing is clicked. Close or move it (`drive.list_windows` finds it) and retry. The wait also covers Linux, where the platform's window-under-cursor updates a frame or two after a warp. Without it, the first click into a different window was hit-tested in the window the pointer had just left, did nothing, and still came back as `no_change_within_budget`. `drive.hover`, `drive.scroll`, `drive.drag` (press point), and `drive.type`/`drive.key` with a handle apply the same gate. `os_input:true` does not, because the X server routes those events itself. Returns the compact `{ outcome, changed, settled, condition_met, elapsed_ms, ticks, input_path, diff (summary), journal? }` — no element list/screenshot unless `observe=list`/`list+screenshot`. On web returns `{ ok, code, diff? (summary) }`.
