...
public class PinWrightPCG : ModuleRules
{
    public PinWrightPCG(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        // Guard for engines that do not ship the PCG plugin at all (custom or stripped
        // engines): fail by name here instead of leaving the consumer with a raw C1083 on a
        // PCG header. This module's sources include those headers directly and ModuleRules
        ...
        // PCG enabled.
        string PCGDir = FindEnginePluginDir(EngineDir, "PCG", new string[]
        {
            "Plugins",                               // UE 5.4-5.8
            Path.Combine("Plugins", "Experimental")  // UE 5.3
        });

        if (string.IsNullOrEmpty(PCGDir))
        {
            throw new BuildException(
                "PinWrightPCG requires the engine plugin 'PCG', which was not found under " +
                Path.Combine(EngineDir, "Plugins") + ". PinWrightPCG is an optional PinWright integration " +
                "sub-module whose sources include PCG headers directly, so it cannot be built against an " +
                "engine that does not ship that plugin. Restore the plugin, or remove the 'PinWrightPCG' " +
                "module entry from PinWright.uplugin and delete Source/PinWrightPCG to build PinWright " +
                "without the PCG integration.");
        }

        PrivateDependencyModuleNames.AddRange(new string[]
        {
            "Core", "CoreUObject", "Engine", "UnrealEd", "Json", "JsonUtilities",
            // Main plugin module: exported handler registration/context and IR sidecar registry seams.
            "PinWright",
            // PCG is hard-linked on purpose: this module uses LoadingPhase None and is only
            // loaded by the main module (LoadModulePtr) when the PCG plugin is enabled, so the
            // main PinWright DLL no longer hard-imports the optional PCG plugin DLLs.
            "PCG", "PCGEditor",
            "AssetRegistry",            // FAssetRegistryModule::AssetCreated in PCGGraphCreate.cpp
            "Projects",                 // IPluginManager in PCGHandlerHelpers.h (optional-plugin gate)
            "EditorScriptingUtilities"  // UEditorAssetLibrary via Tests/TestUtils.h
        });

        // FInstancedPropertyBag (PCG graph parameters) lives in the standalone
